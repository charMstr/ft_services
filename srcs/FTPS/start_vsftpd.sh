...
for i in $USERS ; do
  ...
  echo -e "$PASS\n$PASS" | adduser -h $FOLDER -s /bin/bash $UID_OPT $NAME
  mkdir -p $FOLDER
...
if [ ! -z "$1" ]; then
  exec "$@"
	...
	#ADD '&' AT THE END AND THEN ADD '&& BASH' INSIDE THE DOCKERFILE AT THE END
	# OF THE ENTRYPOINT command if you want to first run this daemon in
	# background and connect to the container in interctive shell.
